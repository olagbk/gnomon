...
var GalleryComponent = (function () {
    ...
    GalleryComponent.prototype.perPageChange = function (response) {
        if (localStorage) {
            localStorage.setItem('galleryPerPage', String(response.perPage));
        }
...
var BlogComponent = (function () {
    ...
    BlogComponent.prototype.perPageChange = function (response) {
        if (localStorage) {
            localStorage.setItem('blogPerPage', String(response.perPage));
        }
        this.perPage = response.perPage;
        if (response.currentPage) {
            this.currentPage = response.currentPage;
...
module.exports = "<div class=\"row justify-content-center\">\n  <span class=\"page-opts align-self-center\">\n    <span class=\"item-count\">{{items}}</span> items</span>\n  <span class=\"page-opts\">\n    <select class=\"per-page-select custom-select\" (change)=\"select($event.target.value)\">\n      <option *ngFor=\"let opt of perPageOpts\" [value]=\"opt\" [selected]=\"opt == perPage\">{{opt}}</option>\n      <option [value]=\"0\" [selected]=\"perPage == 0\">all</option>\n    </select>per page\n  </span>\n  <div class=\"col-12 px-0 pt-md-0\" aria-label=\"album pages\">\n    <pagination [totalItems]=\"items\" [(ngModel)]=\"page\" [maxSize]=\"maxSize\"\n                [boundaryLinks]=\"true\" [rotate]=\"false\" [itemsPerPage]=\"perPage\" (pageChanged)=\"pageChanged($event)\"\n                previousText=\"&lsaquo;\" nextText=\"&rsaquo;\" firstText=\"&laquo;\" lastText=\"&raquo;\"></pagination>\n  </div>\n</div>\n\n"
...
var PaginationComponent = (function () {
    ...
    PaginationComponent.prototype.select = function (perPage) {
        var response = { perPage: perPage };
        var numPages = Math.ceil(perPage / this.items);
...
], PaginationComponent.prototype, "perPageOpts", void 0);
__decorate([
    Object(__WEBPACK_IMPORTED_MODULE_0__angular_core__["Output"])(),
